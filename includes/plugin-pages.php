...
if( !class_exists( 'vxc_pages_hubspot' ) ) {

/**
* @since       1.0.0
*/
class vxc_pages_hubspot   extends vxc_hubspot{  
...
if($post_type == 'shop_order'){
  ...
  */
  ...
  $msg=__('You do not have permissions to export logs','wp-hubspot-woocommerce');
 ...
 $sql="select * $sql_end limit 3000";
  ...
  $fields=array(); $field_titles=array(__("SNO",'wp-hubspot-woocommerce'),__("Status",'wp-hubspot-woocommerce'),__("HubSpot ID",'wp-hubspot-woocommerce') ,__("Order ID",'wp-hubspot-woocommerce'),__("Description",'wp-hubspot-woocommerce'),__("HubSpot Link",'wp-hubspot-woocommerce'),__("Time",'wp-hubspot-woocommerce'));
  ...
  $msg=__('You do not have permissions to clear logs','wp-hubspot-woocommerce');
  $this->display_msg('admin',$msg);
  return;   
  }
  global $wpdb;
  ...
  */
  ...
  if(isset($_GET[$this->id."_logs"])){
      $msg=__('Error While Clearing HubSpot Logs','wp-hubspot-woocommerce');
      $level="error";
      if(!empty($_GET[$this->id."_logs"])){
      $msg=__('HubSpot Logs Cleared Successfully','wp-hubspot-woocommerce');   
      $level="updated";
      }
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'wp-hubspot-woocommerce') . '</a>' );
        }
        return $links;
    } 
    /**
  ...
  */
  ...
  $sections=array(""=>__('HubSpot Settings','wp-hubspot-woocommerce'),'vxc_uninstall'=>__('Uninstall','wp-hubspot-woocommerce'));
  ...
  */
  ...
  $settings_tabs[$this->id] = __( 'HubSpot', 'wp-hubspot-woocommerce' );
        }
  ...
  */
  ...
  add_submenu_page( 'woocommerce', __( 'HubSpot Log','wp-hubspot-woocommerce'),  __( 'HubSpot Log','wp-hubspot-woocommerce') , $this->id."_read_logs", $this->id.'_log', array( $this, 'log_page' ) );
  }
 ...
 include_once(self::$path . "includes/install.php"); 
  do_action('uninstall_vx_plugin_'.$this->type);
   $install=new vxc_install_hubspot();
  $install->remove_data();
  ...
  */
  ...
  array(
  'labels' => array(
  'name'                  => __( 'HubSpot', 'wp-hubspot-woocommerce' ),
  'singular_name'         => __( 'HubSpot Feed', 'wp-hubspot-woocommerce' ),
  'menu_name'             => _x( 'HubSpot Feeds', 'Admin menu name', 'wp-hubspot-woocommerce' ),
  'add_new'               => __( 'New HubSpot Feed', 'wp-hubspot-woocommerce' ),
  'add_new_item'          => __( 'Add New HubSpot Feed', 'wp-hubspot-woocommerce' ),
  'edit'                  => __( 'Edit', 'wp-hubspot-woocommerce' ),
  'edit_item'             => __( 'Edit HubSpot Feed', 'wp-hubspot-woocommerce' ),
  'new_item'              => __( 'New HubSpot Feed', 'wp-hubspot-woocommerce' ),
  'view'                  => __( 'View HubSpot Feed', 'wp-hubspot-woocommerce' ),
  'view_item'             => __( 'View HubSpot Feed', 'wp-hubspot-woocommerce' ),
  'search_items'          => __( 'Search HubSpot Feeds', 'wp-hubspot-woocommerce' ),
  'not_found'             => __( 'No HubSpot Feeds found', 'wp-hubspot-woocommerce' ),
  'not_found_in_trash'    => __( 'No HubSpot Feeds found in trash', 'wp-hubspot-woocommerce' ),
  'parent'                => __( 'Parent HubSpot Feed', 'wp-hubspot-woocommerce' )
  ),
  'description'         => __( 'This is where feeds are stored.', 'wp-hubspot-woocommerce' ),
  'public'              => false,
  'show_ui'             => true,
  'capability_type'     => $this->id,
  //  'capabilities'=>array('read_vxc_hubspot1s'),
  'map_meta_cap'        => true,
  ...
  */
  ...
  1 => __( 'HubSpot Feed updated.', 'wp-hubspot-woocommerce' ),
  6 => __( 'HubSpot Feed updated.', 'wp-hubspot-woocommerce' )
  );
  return $messages;
  } 
  /**
  ...
  */
  ...
  add_meta_box(
  $this->id."_crm_box", //$id
  __( 'HubSpot', 'wp-hubspot-woocommerce' ), //$title
  ...
  */
  ...
  __( 'HubSpot Fields Mapping', 'wp-hubspot-woocommerce' ), //$title
...
if(!empty($v['placeholder'])){ $label=$v['placeholder']; }
 ...
 $user=array('__vx_wp-user_login'=>array('label'=>'user_login'),'__vx_wp-ID'=>array('label'=>'user_id')
  ...
  $tx_arr=array();
  $product_attrs=array('title'=>'Product Title','description'=>'Product Description','short_description'=>'Short Description','sku'=>'Product SKU','price'=>'Product Price','regular_price'=>'Product Regular Price','sale_price'=>'Product Sales Price','total_sales'=>'Product Total Sales','stock_quantity'=>'Stock Quantity','weight'=>'Product Weight','length'=>'Product Length','get_category_ids'=>'Product Categories','get_category'=>'Product First Category','product_img'=>'Product Image URL');
...
$item_fields=array('__vxp_iun-name'=>array('label'=>'Item Name'),'__vxp_iun-type'=>array('label'=>'Item Type'),'__vxp_iun-id'=>array('label'=>'Item ID'));
  ...
  * @param mixed $crm
  */
  private function get_field_mapping($feed,$info="",$object=""){ ///update_post_meta($post_id,'vxc_hubspot_meta',''); 
...
if(!is_array($feed)){ $feed=array(); }
  ...
  ?>
  <div class="alert_danger"><?php echo sprintf(__('HubSpot Settings are not Valid. Go to %sSettings%s','wp-hubspot-woocommerce'),'<a href="'.$link.'">','</a>')?></div>
  <?php
  return;
  }
  ...
  ?>
  <div class="alert_danger"><?php esc_html_e('Please Select Object','wp-hubspot-woocommerce')?></div>
  <?php
  return;
  }
...
$meta= is_array($meta) ? $meta : array(); 
 ...
 $fields=$this->post('fields',$feed); 
  ...
  $fields=$fields == "" ? __("No Fields Found",'wp-hubspot-woocommerce') : $fields;   
   ?>
  <div class="vx_error"><?php echo wp_kses_post($fields) ?></div>
  <?php
  return;
  }
  $sel_fields=array(""=>__("Standard Field",'wp-hubspot-woocommerce'),"custom"=>__("Custom Field",'wp-hubspot-woocommerce'),"value"=>__("Custom Value",'wp-hubspot-woocommerce'));
  ...
  $options_empty=$this->wc_select();
   ...
   $events=array();
     $events["submit"]=__("When user submits the order",'wp-hubspot-woocommerce');
  ...
  $events["manual"]=__("Manually send order to Hubspot",'wp-hubspot-woocommerce');  
   if(self::$is_pr){
//  $events['save_product']=__('Woocommerce Product Updated/Created','wp-hubspot-woocommerce');
  $events['save_user']=__('Wordpress User Updated/Created','wp-hubspot-woocommerce');
  if(class_exists('WC_Subscriptions')){
 ...
 * @param mixed $object
 */
  public function get_object_feeds($object="",$account=""){
        //get feeds of a form
if(empty($this->feeds)){
  ...
  */
  ...
  $html="<option value=''>".esc_html__("Select Object",'wp-hubspot-woocommerce')."</option>";
  $res=array();
  ...
  $res['html']=$html;
      ...
      if(empty($modules)){
          $modules=__('No Objects Found','wp-hubspot-woocommerce');
      }
  $res['error']=$modules;   
  }
...
public function send_order_admin($post_id,$post_type=''){
 ...
 $action= $send_to_sf_button ? '' : 'update';
  ...
  $res=array("class"=>"error","msg"=>__("Nothing Posted to Hubspot",'woo-hubspot-crm-perks'));      
  }
...
$this->send_order_admin($id);
 ...
 $fields=$this->post('fields',$meta);
  ...
  if(empty($primary_key)){
    $primary_key= __("N/A",'wp-hubspot-woocommerce'); 
  }
  }
  ...
  if(empty($object)){
      $object=__("No Object",'wp-hubspot-woocommerce');
  }    
  }
  }
  switch ( $column ) {
  ...
  */
  ...
  $columns['vxc_object'] = __( 'HubSpot Object', 'wp-hubspot-woocommerce' );
  $columns['vxc_key'] = __( 'Primary Key', 'wp-hubspot-woocommerce' );
  ...
  */
  ...
  $this->screen_msg('updated',sprintf(__('Successfully Deleted %d Item(s)','wp-hubspot-woocommerce'),$count));  
    }
  ...
  $bulk_actions=array(""=>__('Bulk Action','wp-hubspot-woocommerce'),"delete"=>__('Delete','wp-hubspot-woocommerce'),
  'send_to_crm_bulk'=>__('Send to HubSpot','wp-hubspot-woocommerce'),'send_to_crm_bulk_force'=>__('Force Send to HubSpot - Ignore Filters','wp-hubspot-woocommerce'));
    $statuses=array(
    "1"=>__('Created','wp-hubspot-woocommerce'),
    "2"=>__('Updated','wp-hubspot-woocommerce'),
    "error"=>__('Failed','wp-hubspot-woocommerce'),
    "4"=>__('Filtered','wp-hubspot-woocommerce'),
    "5"=>__('Deleted','wp-hubspot-woocommerce')
    );
    $menu_links=array(
    array("title"=>__("HubSpot Settings",'wp-hubspot-woocommerce'),
    "link"=>$settings_link
    ),
    array("title"=>__("HubSpot Feeds",'wp-hubspot-woocommerce'),
    "link"=>$feeds_link
    ) );
...
include_once(self::$path."templates/log-entries.php");
 ...
 if($objects == ''){
  ...
  $desc.=" Web2".$row['object'];
   ...
   $desc=sprintf(__('%s filtered','wp-hubspot-woocommerce'),$row['object']);   
  }else   if($row['status'] == 5){
   $desc=sprintf(__('%s deleted','wp-hubspot-woocommerce'),$row['object']);  
  }else{
  $desc.=$row['object'];
  }
  }else{
  ...
  $title=__("Failed",'wp-hubspot-woocommerce');   
  if( $row['status'] == 1){
  $title=__("Created",'wp-hubspot-woocommerce');   
  }else if($row['status'] == 2){
  $title=__("Updated",'wp-hubspot-woocommerce');   
  }else if($row['status'] == 4){
  $title=__("Filtered",'wp-hubspot-woocommerce');   
  }else if($row['status'] == 5){
  $title=__("Deleted",'wp-hubspot-woocommerce');   
  }
  $row['_crm_id']= $crm_id;
  $row['a_link']=$link;
  $row['desc']=$desc;
...
$sql.=' order_id=%d order by id desc limit %d';
  ...
  */
  ...
  $msg=__( 'Successfully Connected to Hub Spot','wp-hubspot-woocommerce' );
     if(isset($info['_time'])){
  ...
  */
  ...
  ?>
  <div class="alert_danger"><?php esc_html_e('Please Select a HubSpot Account','wp-hubspot-woocommerce')?></div>
  <?php
  die();
  }
  $this->post_id=$id=$this->post('id');
...
if(!is_array($feed)){ $feed=array(); }
  ...
  $modules=array(""=>__("Select Object",'wp-hubspot-woocommerce'));
...
public function settings_tab(){
  ...
  $msg=__('You do not have permissions to uninstall','wp-hubspot-woocommerce');
  $this->display_msg('admin',$msg);
  return;   
  }
...
include_once(self::$path."templates/uninstall.php");
 ...
 $id=$this->post('id');
  ...
  $msg=__('Connection to HubSpot is Working','wp-hubspot-woocommerce');
  
  if($conn_class != "updated" ){
      $msg=__('Connection to HubSpot is NOT Working','wp-hubspot-woocommerce');  
  }
  $title=__('Test Connection: ','wp-hubspot-woocommerce');
...
$res=$this->del_account($id);
 if($res){
       $msg=__('Account Deleted Successfully','wp-hubspot-woocommerce');
  $msg_arr=array('msg'=>$msg,'class'=>'updated');   
 }else{
       $msg=__('Error While Removing Account','wp-hubspot-woocommerce');
...
die();
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to add token','wp-hubspot-woocommerce');
  $this->display_msg('admin',$msg);
  return;   
  }
  $id=$this->post('id');
...
$info=$api->handle_code();
  ...
  self::$tooltips = array(
  'sel_object' =>  __('Select the HubSpot object you would like to add your contacts to.', 'wp-hubspot-woocommerce'),
  'map_fields' =>  __('Associate your HubSpot fields to the appropriate order form fields by selecting.', 'wp-hubspot-woocommerce'),
  'optin_condition' =>__('When custom filter is enabled, orders will only be exported to HubSpot when all conditions match. When disabled all orders will be exported.', 'wp-hubspot-woocommerce'),
  'manual_export' => __('Select which WooCommerce event will automatically export the orders into HubSpot. To manually send an order into HubSpot, go to Orders, select the order, and click on the "Send to HubSpot" button.','wp-hubspot-woocommerce'),
  'vx_oauth' => __('OAuth 2.0 is a industry-standard protocol for authorization', 'wp-hubspot-woocommerce'),
 
  'vx_api' => __('Get HubSpot API key from Account Menu -> Integrations -> Get your API Key', 'wp-hubspot-woocommerce'),
  
  'vx_custom_app'=>__('This option is for advanced users who want to override default HubSpot App.','wp-hubspot-woocommerce'),
  
  'vx_disable_logs'=>__('When an order is sent to HubSpot we store that order information in the database and show it in the HubSpot Log. Check this box if you do not want to save the exported order information in the logs.','wp-hubspot-woocommerce'),
  

  'vx_lists'=>__('Get lists from Hub Spot.','wp-hubspot-woocommerce'),

  'vx_sel_list'=>__('Which List should be assigned to this object.','wp-hubspot-woocommerce'),
  'vx_list_check'=>__('If enabled, Contact will be added to selected List','wp-hubspot-woocommerce'),
  
  'vx_flows'=>__('Get Work Flows from Hub Spot.','wp-hubspot-woocommerce'),

  'vx_sel_flow'=>__('Which Work Flow should be assigned to this object.','wp-hubspot-woocommerce'),
  'vx_flow_check'=>__('If enabled, Contact will be added to selected Work Flow','wp-hubspot-woocommerce'),
  
   'vx_assign_contact'=>__('Enable this option if you want to assign a Contact this object.','wp-hubspot-woocommerce'),
   'vx_assign_company'=>__('Enable this option if you want to assign a Company this object.','wp-hubspot-woocommerce'),
  
   'vx_sel_contact'=>__('Select Contact feed. Contact created by this feed will be assigned to this object','wp-hubspot-woocommerce'),
   'vx_sel_company'=>__('Select Contact feed. Company created by this feed will be assigned to this object','wp-hubspot-woocommerce'),
   
 
   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','wp-hubspot-woocommerce'),
   'vx_owners'=>__('Get Users list from HubSpot','wp-hubspot-woocommerce'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to HubSpot Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the HubSpot Object selected in the feed.','wp-hubspot-woocommerce'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','wp-hubspot-woocommerce'),
   'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note','wp-hubspot-woocommerce'),
   'vx_note_fields'=>__('Select fields which you want to send as a note','wp-hubspot-woocommerce'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry','wp-hubspot-woocommerce')
...
$fields=array("fields"=>$this->post('fields',$meta_post));
 ...
 if(!empty($info['meta']) ){
  ...
  switch($action){
    ...
    if(is_array($camps)){
    $res['status']="ok";
    $data['crm_sel_list']=$this->gen_select($camps,$status_sel,__('Select List','wp-hubspot-woocommerce'));
    $meta['lists']=$camps;  
  ...
  $res['data']=$data;   
    ...
    if(is_array($camps)){
    $res['status']="ok";
    $data['crm_sel_pipe']=$this->gen_select($camps,$status_sel,__('Select Pipeline','wp-hubspot-woocommerce'));
    $meta['pipes']=$camps;  
  ...
  $path='crm-objects/v1/objects/products/paged?properties=name';
  if(!empty($offset)){
      $path='&Offset='.$offset;
  }
  $res['status']="ok";
  $arr=$api->post_hubspot_arr($path);
 //var_dump($arr);
  if(!empty($arr['objects'])){
      foreach($arr['objects'] as $v){
          if(!empty($v['properties']['name']['value'])){
          $id=$v['objectId'];
      $post=get_page_by_title($v['properties']['name']['value'], ARRAY_A, 'product');
      if(!empty($post['ID'])){
        update_post_meta($post['ID'],'vxc_hubspot_id',$id);  
      }
          }    
      }
  }else  if(isset($arr['message'])){
   $res['error']=$arr['message'];   
  }
  $meta['last_product_check']=current_time( 'timestamp');
  $res['page']=$page+=1;
  if(isset($arr['hasMore']) && $arr['hasMore'] == true){
  ...
  case"refresh_flows":
    ...
    if(is_array($camps)){
    $res['status']="ok";
    $data['crm_sel_flow']=$this->gen_select($camps,$status_sel,__('Select Work Flow','wp-hubspot-woocommerce'));
    $meta['flows']=$camps;  
  ...
  case"refresh_users": 
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','wp-hubspot-woocommerce'));
    $meta['users']=$users;   
    }else{
     $res['error']=$users;   
    }
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','wp-hubspot-woocommerce');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  ...
  if(!$valid_email){
      $this->screen_msg('error',__('Invalid Email(s)','wp-hubspot-woocommerce'));
  }
  ...
  $msg="";
  if(empty($app_id)){
  $msg=__('Please Enter APP ID','wp-hubspot-woocommerce');    
  }
  if(empty($app_secret)){
  $msg=__('Please Enter APP Secret','wp-hubspot-woocommerce');    
  }
    if(empty($app_url)){
  $msg=__('Please Enter APP URL','wp-hubspot-woocommerce');    
  }
    if(!empty($msg)){
