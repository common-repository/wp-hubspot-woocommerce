...
ul.vx_notes li.customer-note .note_content:after {
  ...
  ?>
  <p>
  <button class="button" type="submit" name="<?php echo esc_attr($this->id) ?>_send" title="<?php esc_html_e("Send to HubSpot",'wp-hubspot-woocommerce')?>" value="yes"><?php esc_html_e("Send to HubSpot",'wp-hubspot-woocommerce')?></button>
  <?php
      if($comments ){
  $log_url=admin_url( 'admin.php?page='.$this->id.'_log&order_id='.$post_id); 
  ?>
  <a href="<?php echo esc_url($log_url) ?>" class="button" title="<?php esc_html_e("Go to Logs",'wp-hubspot-woocommerce')?>"><?php esc_html_e("Go to Logs",'wp-hubspot-woocommerce')?></a>
  <?php
      }
  ?>
  </p>