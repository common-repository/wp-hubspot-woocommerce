...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <h2 style="padding-bottom: 12px; line-height: 36px">  <img alt="<?php esc_html_e("HubSpot Feeds", 'wp-hubspot-woocommerce') ?>" title="<?php esc_html_e("HubSpot Feeds", 'wp-hubspot-woocommerce') ?>" src="<?php echo esc_attr(self::$base_url)?>images/<?php echo esc_attr($this->crm_name) ?>-crm-logo.png" style="float:left; margin:0 7px 0px 0;" width="40" /> <?php esc_html_e("HubSpot Log", 'wp-hubspot-woocommerce'); ?></h2>
...
<form id="vx_form" class="crm_form" method="get"><div>
  ...
  <input type="text" placeholder="<?php esc_html_e('Search','wp-hubspot-woocommerce') ?>" value="<?php echo esc_attr($this->post('search')) ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php esc_html_e('All Objects','wp-hubspot-woocommerce') ?></option>
  <?php
  ...
  <option value=""><?php esc_html_e('All Status','wp-hubspot-woocommerce') ?></option>
  <?php    
  ...
  <option value=""><?php esc_html_e('All Times','wp-hubspot-woocommerce') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php esc_html_e('From Date','wp-hubspot-woocommerce') ?>" value="<?php if(isset($_REQUEST['start_date'])){echo esc_attr($this->post('start_date'));}?>" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="<?php if(isset($_REQUEST['end_date'])){echo esc_attr($this->post('end_date'));}?>" placeholder="<?php esc_html_e('To Date','wp-hubspot-woocommerce') ?>" name="end_date"  style="width: 100px">
  </span>
 
  <button type="submit" class="button-secondary button crm_input_inline" name="search" title="<?php esc_html_e('Search','wp-hubspot-woocommerce') ?>"><i class="fa fa-search"></i> <?php esc_html_e('Search','wp-hubspot-woocommerce') ?></button> 
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php esc_html_e('Apply','wp-hubspot-woocommerce') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php esc_html_e('Apply','wp-hubspot-woocommerce') ?></button>

  <?php          if(count($results)>0){
  ?>
  <button type="button" name="tab_action" id="vx_export" title="<?php esc_html_e('Export as CSV','wp-hubspot-woocommerce') ?>" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php esc_html_e('Export as CSV','wp-hubspot-woocommerce') ?></button> 
  <?php
  ...
  if(isset($results[0]['order_id']) && $results[0]['order_id']!=""){
     ...
     $parent_id=(int)$results[0]['parent_id'];
      ?>
  <a href="<?php echo esc_url(admin_url( 'admin.php?page='.$this->id.'_log&order_id='.$order_id));?>" title="<?php echo sprintf(__('View Order# %s Logs','wp-hubspot-woocommerce'),esc_attr($order_id)); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php 
  if($parent_id<0){
  echo sprintf(__('View Visitor# %s Logs','wp-hubspot-woocommerce'),esc_attr($order_id)); 
  }else{
  echo sprintf(__('View Order# %s Logs','wp-hubspot-woocommerce'),esc_attr($order_id)); 
  }
   ?></a><?php
  }}
 if($this->post('order_id') !="" || $this->post('id') !=""){
          ?><a href="<?php echo esc_url(admin_url( 'admin.php?page='.$this->id.'_log'));?>" title="<?php esc_html_e('View All Logs','wp-hubspot-woocommerce') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php esc_html_e('View All Logs','wp-hubspot-woocommerce') ?></a>        
          <?php
      }
  ?>
  <div class="tablenav-pages">
  <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','wp-hubspot-woocommerce') ?> <span id="paging_range_min_header"><?php echo esc_attr($range_min)?></span> - <span id="paging_range_max_header"><?php echo esc_attr($range_max); ?></span> of <span id="paging_total_header"><?php echo esc_attr($items);?></span></span>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php esc_html_e('HubSpot ID', 'wp-hubspot-woocommerce') ?>
  <i class="fa fa-caret-<?php echo esc_attr($crm_order) ?> vx_sort_icon <?php echo esc_attr($crm_class) ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="order_id"><?php esc_html_e('Order ID', 'wp-hubspot-woocommerce') ?>
  <i class="fa fa-caret-<?php echo esc_attr($entry_order) ?> vx_sort_icon <?php echo esc_attr($entry_class) ?>"></i>                      
  </th>
  <th scope="col" class="manage-column"  data-name="object"><?php esc_html_e('Description', 'wp-hubspot-woocommerce') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php esc_html_e('Time', 'wp-hubspot-woocommerce') ?>
  <i class="fa fa-caret-<?php echo esc_attr($time_order) ?> vx_sort_icon <?php echo esc_attr($time_class) ?>"></i>
  </th>
  <th style="width: 40px"><?php esc_html_e('Detail','wp-hubspot-woocommerce') ?></th>
  </tr>
  </thead>
  ...
  <th><?php esc_html_e('Detail','wp-hubspot-woocommerce') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo esc_attr($row['id']) ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo esc_attr(self::$base_url) ?>images/<?php echo esc_attr($row["status_img"]) ?>.png" alt="<?php echo $row['status'] ? __('Active', 'wp-hubspot-woocommerce') : __('Inactive', 'wp-hubspot-woocommerce');?>" title="<?php echo esc_html($row['title']);?>" class="crm_status_img" /></td>
  <td class="column-name" style="width:40%"><?php echo wp_kses_post($row['a_link']) ?></td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php esc_html_e('Expand Detail','wp-hubspot-woocommerce'); ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php esc_html_e('No Record(s) Found', 'wp-hubspot-woocommerce'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php esc_html_e('Clear HubSpot Log','wp-hubspot-woocommerce') ?>" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce')); ?>"><?php esc_html_e('Clear HubSpot Log','wp-hubspot-woocommerce') ?></a>
  <div class="tablenav-pages">   <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','wp-hubspot-woocommerce') ?> <span id="paging_range_min_header"><?php echo esc_attr($range_min)?></span> - <span id="paging_range_max_header"><?php echo esc_attr($range_max); ?></span> of <span id="paging_total_header"><?php echo esc_attr($items);?></span></span>
  <?php  echo wp_kses_post($page_links);?></div>
    </div>
  <?php
  }
...
if(sel.val() == ""){
    alert('<?php esc_html_e('Please Select Action','wp-hubspot-woocommerce') ?>');
      return false;
}
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php esc_html_e('Please select at least one entry','wp-hubspot-woocommerce') ?>');
    return false;
}
var action=sel.val();
if( $.inArray(action,["send_to_crm_bulk_force","send_to_crm_bulk"]) !=-1 && $(".crm_input_check:checked").length>4){
 if(!confirm('<?php esc_html_e('Exporting more than 4 entries may take too long. Are you sure you want to continue?','wp-hubspot-woocommerce') ?>')){
  e.preventDefault();    
 }   
}
  })
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php esc_html_e('HubSpot Logs will be deleted permanently. Do you want to continue?','wp-hubspot-woocommerce') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(is_main){
    icon.attr('title','<?php esc_html_e('Expand Detail','wp-hubspot-woocommerce'); ?>');    
    }        
     return;   
    }else{
        next_tr.show('fast'); 
    next_tr.find('.vxa_entry').slideDown('fast');
       if(is_main){
    icon.attr('title','<?php esc_html_e('Collapse Detail','wp-hubspot-woocommerce'); ?>');    
    }    
    }
