...
.crm_panel .vx_error{
  ...
  <div id="crm_fields_map" class="vx_contents">
    <div class="vx_div ">
  <div class="vx_head ">
<div class="crm_head_div"> <?php esc_html_e('1. Select HubSpot Account',  'wp-hubspot-woocommerce' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vxc_account"><?php esc_html_e('Select HubSpot Account.', 'wp-hubspot-woocommerce');  $this->tooltip($tooltips['sel_object']); ?></label>
  </div>
  <div class="vx_col2">
  <div>
  ...
  <div id="crm_ajax_account" style="display:none; text-align: center; line-height: 100px;"><i><?php esc_html_e('Loading, Please Wait...', 'wp-hubspot-woocommerce'); ?></i></div>
    <?php 
    $valid=true;
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','wp-hubspot-woocommerce') ?></div></div>');
    return false;
    }  
   })  
 }
 ...
 if(re.status == "ok"){
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','wp-hubspot-woocommerce') ?>');
     return;   
    }  
...
str+='<option value="'+k+'">'+label+'</option>';    
});
str+='</select>';    
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  e.preventDefault(); 
  if(!confirm('<?php esc_html_e('Are you sure to remove ?','wp-hubspot-woocommerce') ?>')){
      return;
  }
...
start_tooltip();
  ...
  if(object == ""){
  alert("<?php esc_html_e('Please Select Object','wp-hubspot-woocommerce') ?>");
  return;
  }
  if(vx_ajax && vx_ajax.abort){
    vx_ajax.abort();  
...
$('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','wp-hubspot-woocommerce') ?>'});
add_note_sel();
} 
  });
function add_note_sel(){
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','wp-hubspot-woocommerce') ?>'});
}  
