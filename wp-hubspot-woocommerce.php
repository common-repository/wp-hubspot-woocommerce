<?php
/*
* Plugin Name: Integration for HubSpot and WooCommerce
* Description: Integrates WooCommerce with HubSpot allowing new orders to be automatically sent to your HubSpot account.
* Version: 1.1.7
* Requires at least: 3.8
* Author: CRM Perks
* Author URI: https://www.crmperks.com
* Plugin URI: https://www.crmperks.com/plugins/woocommerce-plugins/woocommerce-hubspot-plugin/
* Text Domain: wp-hubspot-woocommerce
...
class vxc_hubspot{            
 public $url='https://www.crmperks.com';           
  ...
  private $temp= '';
  public static $note= '';
  public static $title='WooCommerce HubSpot Plugin';  
...
add_action('woocommerce_saved_order_items',array($this,'save_lines'),999,2);
  ...
  load_plugin_textdomain('wp-hubspot-woocommerce', FALSE, $this->plugin_dir_name() . '/languages/' );
  ...
  $log_str="Installing WooCommerce HubSpot Plugin version=".$this->version;
...
*/
  ...
  $message = sprintf(__("%sWooCommerce%s is required. %sDownload latest version!%s", 'wp-hubspot-woocommerce'), "<a href='https://www.woothemes.com/woocommerce/'>", "</a>", "<a href='https://www.woothemes.com/woocommerce/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('WooCommerce is installed but not active. %sActivate WooCommerce%s to use the WooCommerce HubSpot Plugin','wp-hubspot-woocommerce'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=woocommerce/woocommerce.php'), 'activate-plugin_woocommerce/woocommerce.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sWooCommerce%s is required. %sDownload latest version!%s", 'wp-hubspot-woocommerce'), "<a href='https://www.woothemes.com/woocommerce/'>", "</a>", "<a href='https://www.woothemes.com/woocommerce/'>", "</a>");
  }  
...
do_action('vx_addons_save_entry',$id,'','wc','');         
  ...
  */
  ...
  return "<div class='alert_danger crm_alert'>".sprintf(__("Please Configure %sHubSpot Settings%s",'wp-hubspot-woocommerce'),"<a href='".$link."'>","</a>")."</div>";
...
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','wp-hubspot-woocommerce') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  ...
  */
  ...
  ?>
  <div class="<?php echo esc_attr($type) ?> fade notice is-dismissible"><p><?php echo wp_kses_post($message);?></p></div>    
  <?php   
  }
    /**
...
return  $var;
  ...
  */
  ...
  if($note['status'] == "4"){
    $msg=sprintf(__('HubSpot (%s) Filtered','wp-hubspot-woocommerce'),$object);    
  }else{
      //web2lead do not have link
  $link=sprintf(__("with ID # %s",'wp-hubspot-woocommerce'),$this->post('id',$note));
  if($this->post('link',$note) !=""){
      $id_link='<a href="'.$this->post('link',$note).'" target="_blank" title="'.$this->post('id',$note).'">'.$this->post('id',$note).'</a>';
  $link=sprintf(__('with ID # %s','wp-hubspot-woocommerce'),$id_link);
  }
...
if($note['status'] == '1'){
  $msg=sprintf(__('Added to HubSpot (%s) ','wp-hubspot-woocommerce'),$object).$link;
  }else{
  $msg=sprintf(__("Updated to HubSpot (%s) ",'wp-hubspot-woocommerce'),$object).$link;     
  }                                         
  }
  }else if($show_error){
     if(empty($error)){
  ...
  $msg.=' - <a href="'.esc_url($log_url).'" class="vx_log_link" title="'.__('View Detail','wp-hubspot-woocommerce').'" data-id="'.esc_html($note['log_id']).'">'.__('View Detail','wp-hubspot-woocommerce')."</a>";
  } 
  return $msg;
  }
  /**
...
if( $status == 'update_order' && !in_array($object,array('Deal'))){ // only allow orders because only order lines updated
   continue;
}
  $feed['id']=$id; 
 
 if(!empty($feed['contact_check']) || !empty($feed['company_check'])){
  if(!empty($feed['contact_check'])){
     $feeds_meta[$e++]=$feed; 
  }else{
  $feeds_meta[$k++]=$feed; 
  }
...
$feed['crm_object']=$feed_log['object'];
...
//var_dump($temp); die();  
  ...
  if(isset($feed['owner']) && $feed['owner'] == "1"){
   $feed['OwnerId']=array('value'=> $feed['user'],'label'=>'Owner ID');   
  }

  //add account or contract

    if(!empty($feed['contact_check']) && !empty($feed['object_contact'])){
     $contact_feed=$feed['object_contact']; 
  ...
  if(!empty($contact_res['id'])){
   $feed['_vx_contact_id']=array('value'=> $contact_res['id'],'label'=>'Contact ID');   
    ...
    if(!empty($feed['company_check']) && !empty($feed['object_company'])){
     $company_feed=$feed['object_company']; 
  ...
  //
     ...
     $entry_note_title=substr($entry_note,0,40);   
  ...
  $api=$this->get_api($info); 
    if(!is_admin() && isset($_COOKIE['hubspotutk'])){
 $feed['_vx_entry']=array('_vx_htuk'=>sanitize_text_field($_COOKIE['hubspotutk']));
 } 
...
//  if($this->post('disable_log',$settings) !="yes"){
  ...
  */
  ...
  $subject=__('Error While Posting to HubSpot','wp-hubspot-woocommerce');
$page_url = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; 
  ...
  $email_info=array("msg"=>$res['error'],"title"=>__("HubSpot Error",'wp-hubspot-woocommerce'),"info_title"=>"More Detail","info"=>array("Order Id"=>$order_url,"Time"=>date('d/M/y H:i:s',current_time('timestamp')),"Page URL"=>'<a href="'.esc_url($page_url).'" style="word-break:break-all;">'.esc_url($page_url).'</a>'));
  ...
  */
  ...
  $status=0;
  if(class_exists('vxc_hubspots_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/wp-hubspot-woocommerce/wp-hubspot-woocommerce.php')) {
  $status=2;
  } 
  return $status;
  }
...
return $result;
  ...
  */
  public function get_objects($info="",$refresh=false){
      ...
      $objects=array("Contact"=>'Contact',"Company"=>'Company',"Deal"=>'Deal',"Ticket"=>'Ticket',"Task"=>'Task');
       if(!empty($info)){   
   $meta=$this->post('meta',$info);  
   }else{
...
$vxc_hubspot=new vxc_hubspot(); 
$vxc_hubspot->instance();
$vx_wc['vxc_hubspot']='vxc_hubspot';
